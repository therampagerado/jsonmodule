...
class jsonModule extends Module
{
     ...
     * @return string
     */
    private function buildJsonLdGraph()
    {
        $graph = [];

        $organizationJson = Configuration::get(static::ORGANIZATION_JSON);
        if ($organizationJson) {
            $organizationNode = json_decode($organizationJson, true);
            if (is_array($organizationNode) && !empty($organizationNode)) {
