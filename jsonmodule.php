...
class jsonModule extends Module
{
    ...
    public function hookDisplayHeader($params)
    {
        ...
        $organizationNode = [];
        $storedOrganization = Configuration::get(static::ORGANIZATION_JSON);
        if ($storedOrganization) {
            $organizationNode = json_decode($storedOrganization, true);
            if (is_array($organizationNode) && isset($organizationNode['@context'])) {
                unset($organizationNode['@context']);
            }
        }

        $graph = [];
        if (!empty($organizationNode)) {
